...
#' @rdname nca
#' @export
nca2 <- function(formula, data, neighborhood, subset, na.action, ...) {
  Call <- match.call()
  ...
  out <- nca2.fit(y = y, X = X, neighborhood = mf[["(neighborhood)"]], ...)
  out$terms <- Terms
...
#' @rdname nca
#' @export
nca2.fit <- function(y, X, n_components, init = c("pca", "identity"), loss = NULL, ...,
                    neighborhood = NULL,
  ...
  calculate_once <- function(A, verbose = debug) {
    ...
    sp <- if(is.na(sparse)) {
      if(is.null(env$pij)) FALSE else spam::is.spam(env$pij[[1]]) || mean(env$pij[[1]] == 0) > 0.7
    } else sparse
    env$pij <- lapply(env$AX, calculate_pij, sparse = sp)
    return(NULL)
  }
  ...
  calculate_objective <- function(A) {
    ...
    calculate_once(A)
    # stopifnot(env$A == A)
    do.call(sum, Map(env$pij, yiyj, f = function(pp, yy) sum(yy * pp)))/N + 0.5*lambda*sum(A^2) # since we're minimizing, we want to *add* the penalty
  }
  ...
  calculate_gradient <- function(A) {
    ...
    tmp <- Map(AX, pij, yiyj, Xsplit, f = function(ax, pp, yy, xx) {
      py <- yy * pp
      pi <- spam::rowSums(py)
      W <- py - matrix(pi, nrow = nrow(pp), ncol = ncol(pp), byrow = FALSE) * pp
      stopifnot(all.equal(spam::rowSums(W), rep.int(0, nrow(pp))))
      W2 <- W + spam::t(W)
