nca_fitted_regression <- function(pp, yy) {
  ...
  spam::rowSums(pp * matrix(yy, nrow = nrow(pp), ncol = ncol(pp), byrow = TRUE))
}

nca_fitted_classification <- function(pp, yy, classes) {
  stopifnot(ncol(pp) == length(yy))
  vapply(classes, function(yyy) {
    spam::rowSums(pp[, yy == yyy])
  }, numeric(nrow(pp)))
}


nca_fitted_regression2 <- function(pp, yy) {
  stopifnot(nca_ncol(pp) == length(yy))
  nca_rowSums(nca_mult(pp, matrix(yy, nrow = nca_nrow(pp), ncol = nca_ncol(pp), byrow = TRUE)))
}
nca_fitted_classification2 <- function(pp, yy, classes) {
  stopifnot(nca_ncol(pp) == length(yy))
  vapply(classes, function(yyy) {
    if(is_nca_sparse(pp)) {
      cols <- which(yy == yyy)
      pp2 <- pp
      pp2$entries <- pp$entries[pp$entries[, 2] %in% cols, ]
      nca_rowSums(pp2)
    } else rowSums(pp[, yy == yyy])
  }, numeric(nca_nrow(pp)))
}
